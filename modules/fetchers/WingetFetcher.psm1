...
function Get-WingetVersion {
    ...
    # We specify --source winget to ensure we are checking the repo, not just local
    # We use --exact to ensure we get the specific package requested
    # Note: 'winget show' might fail if the package is not found
    try {
        # Using --exact to prevent ambiguity
    ...
    $Version = $null
    $Url = "winget install --id $PackageId" # Default action since URL is hard to get via CLI
    ...
    # Winget CLI doesn't easily give the release date or direct URL in 'show' without --manifest
...
function Update-WingetComponent {
    ...
    try {
        # Command: winget upgrade --exact --id <id> --silent --accept-source-agreements --accept-package-agreements
        # Note: --silent might require admin privileges depending on the package.
