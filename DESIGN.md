...
*   **Output:**
    *   `[PSCustomObject]`: `{ IsUpdateAvailable (bool), Status (string), Diff (string) }`
...
*   **Functions:** `Get-SteamManifestVersion`, `Get-EpicManifestVersion`
*   **Logic:**
...
*   **Logic:**
    ...
    *   Delegates to `WingetFetcher` to perform the update.
*   **Output:**
    *   The updated component object with `UpdateStatus` populated.

## 5. Error Handling Strategy
To ensure robustness, the system uses a **Per-Component Isolation** strategy.

1.  **Try/Catch Blocks:** The Orchestrator wraps the processing of *each* component in a `try/catch` block.
2.  **State Tracking:** If a module fails (e.g., `VersionFetcher` cannot reach GitHub):
    ...
    *   The component's `ErrorMessage` is populated with a user-friendly description.
3.  **Pipeline Continuation:** The failure of one component does not stop the execution of others.
4.  **Reporting:** The final HTML report highlights checks that failed, ensuring the user is aware of incomplete data.
